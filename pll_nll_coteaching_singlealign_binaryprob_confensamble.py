...
def pick_smallloss_momconf(nll_losses, rc_losses, align_losses, co_lambda, remember_rate, batch_confidence,
                           batch_momconf, noise_or_not):
    loss_pick = nll_losses * (1 - co_lambda) + co_lambda * align_losses
    ind_sorted = torch.argsort(loss_pick)
    loss_sorted = loss_pick[ind_sorted]
    num_remember = int(remember_rate * loss_sorted.shape[0])
    ind_update = ind_sorted[:num_remember]
    smallloss_pureratio = torch.mean(noise_or_not[ind_update].float())

    mask = np.argmax(batch_confidence, axis=1)[ind_update.cpu().numpy()] == np.argmax(batch_momconf, axis=1)[
        ind_update.cpu().numpy()]
    ind_update = ind_update[mask]  # 选择动量 confidence 和 当前 confidence 所指示标签相同的样本
    final_pure_ratio = torch.mean(noise_or_not[ind_update].float())

    # 保留部分
    losses = nll_losses + rc_losses + align_losses
    # losses = nll_losses + rc_losses # 去掉关于对齐损失的优化
    picked_losses = torch.mean(losses[ind_update])
    picked_nll_losses = torch.mean(nll_losses[ind_update])
    picked_rc_losses = torch.mean(rc_losses[ind_update])
    picked_align_losses = torch.mean(align_losses[ind_update])
    # logging.info('Number of selected samples: samllloss:{}, plus momconf:{}'.format(num_remember, len(ind_update)))
    return picked_losses, smallloss_pureratio, final_pure_ratio, picked_nll_losses, picked_rc_losses, picked_align_losses, len(
        ind_update)
...
def DPLL_coteaching_train(train_loader, model_a, model_b, optimizer, epoch, consistency_criterion, confidence,
    ...
    for i, (x_aug0, x_aug1, x_aug2, y, part_y, index) in enumerate(train_loader):
        ...
        # 20230520 修改：之前忘记添加 picked_nll_loss
        final_loss = super_loss_a + selftrain_loss_b + lam * (
                consist_loss.mean() + alignlosses.mean() + sim_struct_loss)
        ...
        # update confidence: 利用 自监督 task 所训练模型计算的 confidence 更新用于训练 PLL 的 confidence
